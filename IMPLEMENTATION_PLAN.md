...
Updated: 2026-09-13. This is the authoritative backlog and execution order.
...
- Home redesign, album wizard, semantic UI migration, and the Persian accessibility/RTL pass are complete. The launch scope is Persian (`fa`) only with RTL layout; `en`, `ar`, and `fr` remain dormant future infrastructure.
- Fresh restore and Release build/test succeeded. The latest complete run passed all 88 cases with no failures or skips. Two seed warnings remain assigned to B13 in `docs/BASELINE_2026-09-11.md`; the duplicate resource warnings are resolved.
...
| B09 | P1 / UI completion | Done | Preserve the approved minimal, content-first blue design. Replace raw palette utilities and inconsistent forms with semantic tokens across public/auth/admin views; standardize loading, empty, error and retry states while preserving routes and editor bindings. | B06, B07 | Passed 2026-09-13: all 126 affected Razor files use the shared semantic palette with no raw named palette utilities; common labels, controls, field errors and empty/loading/error states are standardized. Full 84-case Release suite passed. Live fa/RTL public and auth checks at 1280px and 375px had Vazirmatn, 44px controls, no overflow or console errors. See docs/B09_UI_COMPLETION_PLAN.md. |
| B10 | P1 / Persian accessibility and RTL | Done | Treat Persian (`fa`) as the only launch culture. Verify Vazirmatn, RTL logical layout, labels, keyboard/focus behavior, errors, contrast, reduced motion and responsive widths 375/768/1024/1440 across home, public pages, auth, admin and wizard. Keep `en`/`ar`/`fr` redirects and dormant localization infrastructure for future work. | B08, B09 | Passed 2026-09-13: skip links, focus-visible styling, live validation, inert/trapped mobile navigation, logical RTL spacing, labeled 44px icon controls, AA token contrast and reduced motion are enforced. Full 88-case Release suite and live home/catalog/auth/admin/wizard checks at 375/768/1024/1440 passed with no overflow or console errors. See docs/B10_PERSIAN_ACCESSIBILITY_PLAN.md. |
| B11 | P1 / Search operations | Verify | Runtime readiness now requires an enabled FTS service and all 14 union tables, with a per-cache-miss probe so a completed index deployment takes effect without restart. Run FullTextSearch.sql plus VerifyFullTextSearch.sql on an FTS-capable SQL Server; verify LIKE fallback, ranked FTS results, paging totals and automatic-change-tracking visibility. | B06; FTS-capable SQL Server | LIKE evidence passed. Both FTS paths must be tested with seeded queries; record indexing delay and maintenance in docs/B11_SEARCH_OPERATIONS_PLAN.md. No recurring rebuild unless a demonstrated operational need exists. |
...
| B16 | P1 / Lapis Archive redesign | In progress | Apply the approved Lapis Archive theme across public, auth, and admin surfaces. Start with a local licensed home-banner asset, then establish semantic tokens and three reference pages (home, album list, album detail) before migrating the remaining views. Preserve routes, home loading behavior, SEO, JSON-LD, Persian RTL, accessibility, and responsive behavior. | B08-B10; approved asset | Build/test baseline plus 375/768/1024/1440 visual evidence, keyboard/RTL/reduced-motion checks, image performance metrics, and documented asset credit. See `LAPIS_ARCHIVE_REDESIGN_PLAN.md`. |

## Implementation batches

1. **Foundation:** B01, B02, B03 are complete.
2. **Correctness:** B04 through B07 are complete.
3. **Experience:** B08, B09, B10 are complete. B16 is the approved visual follow-up: use the Lapis Archive plan and preserve the existing home behavior rather than rebuilding its data or pagination contract.
...
- `UI_REDESIGN_PLAN.md`: visual specifications and completed B09/B10 evidence.
- `LAPIS_ARCHIVE_REDESIGN_PLAN.md`: approved theme direction and implementation sequence for B16.
