...
The current Persian-only middleware redirects `en`, `ar`, and `fr` routes to `fa`; this is the approved launch policy. B06 verifies database localization overlays through the reachable `fa` route. B10 subsequently completed Persian RTL and accessibility verification, while multilingual delivery remains deferred.
